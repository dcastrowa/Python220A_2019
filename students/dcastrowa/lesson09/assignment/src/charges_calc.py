...
def conditional_logging(func):
    ...
    def init_logging(logging_level, *args):
        """
        init logging for debug command options
        :param logging_level:
        :return: logger
        """
    ...
    for value in json_data.values():
        try:
            ...
            if rental_end < rental_start:
                logging.error(f'The end date {value["rental_end"]}'
                              f' shouldn\'t be before'
                              f' the start date {value["rental_start"]}.')
            elif rental_start == rental_end:
                logging.warning(f'Returned the same day')
            else:
            ...
            if value['units_rented'] == 0:
                logging.error(f"No units rented, can't calculate unit_cost. "
